...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container == self.is_container or (place.ant.is_container and place.ant.ant_contained != None):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        find_nearest_bee = None
        find_place = self.place
        distance = 0
        while not find_place.is_hive :
            if len(find_place.bees) :
                if find_nearest_bee == None and self.lower_bound <= distance <= self.upper_bound:
                    find_nearest_bee = random_bee(find_place.bees)
                    break
            find_place = find_place.entrance
            distance += 1
        return find_nearest_bee
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0 :
            cost = cost + self.damage

        for bee in list(self.place.bees) :
            bee.reduce_health(cost)
...
class HungryAnt(Ant) :
    ...
    def action (self, gamestate) :
        if len(self.place.bees) and self.cooldown == 0:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        checking_place = self.place.exit
        while checking_place != None :
            if checking_place.ant != None and checking_place.ant.is_doubled == False :
                checking_place.ant.double()
                checking_place.ant.is_doubled = True
            if checking_place.ant != None and checking_place.ant.is_container and checking_place.ant.ant_contained != None and checking_place.ant.ant_contained.is_doubled == False:
                checking_place.ant.ant_contained.double()
                checking_place.ant.ant_contained.is_doubled = True
            checking_place = checking_place.exit
        # END Problem 12
