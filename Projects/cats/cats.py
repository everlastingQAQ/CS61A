...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def check (string) :
        lower_string = lower(remove_punctuation(string))
        split_lower_strings = split(lower_string)
        have = False
        for inchecking_string in split_lower_strings :
            for check_string in subject :
                if (lower(check_string) == inchecking_string) :
                    have = True
        return have
    return check
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    for i in range (typed_len) :
        ...
        if (typed_words[i] == source_words[i]) :
            is_same += 1
    correct_rate = 100 * is_same / typed_len
    return correct_rate
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    typed_len = len(typed)
    count_words = typed_len / 5
    d = 60 / elapsed
    type_rate = count_words * d
    return type_rate
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list :
        ...
        if (cur_diff < mn_diff and cur_diff <= limit) :
            mn_diff = cur_diff
            upcoming_word = word
    if (mn_diff == limit + 1) :
        return typed_word
...
def furry_fixes(typed, source, limit):
    ...
    mn_len = min(type_len, source_len)

    diff = abs(type_len - source_len)
    def cal (cur_idx) :
        nonlocal diff
        if (cur_idx == mn_len) :
            return
        if (typed[cur_idx] != source[cur_idx]) :
            diff += 1
        if (diff > limit) :
            return
        cal(cur_idx + 1)
...
def time_per_word(words, timestamps_per_player):
    ...
    for per_tpp in tpp :
        cur = []
        for i in range(len(per_tpp)) :
            if (i != 0) :
                cur.append(per_tpp[i] - per_tpp[i - 1])
        times.append(cur)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word in word_indices :
        ...
        for player in player_indices :
            if (times[fastest_player][word] > times[player][word]) :
                fastest_player = player
        fastest[fastest_player].append(words[word])
