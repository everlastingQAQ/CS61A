...
def boar_brawl(player_score, opponent_score):
    ...
    cur = abs(opponent_tens_digit - player_ones_digit) * 3
    cur = max(1, cur)
    return cur
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    points = player_score + take_turn(num_rolls, player_score, opponent_score, dice)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    Is_always_roll = True
    ini_counts = strategy(0, 0)
    for player_score in range (goal) :

        for opponent_score in range (goal) :
            if (strategy(player_score, opponent_score) != ini_counts) :
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged_dice (*arg) :
        ...
        for times in range(times_called) :
            sum_of_all += original_function(*arg)

        ave = sum_of_all / times_called
        return ave
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num_rolls in range (1, 11) :
        cur = averaged_dice(num_rolls, dice)
        if (cur > ave) :
            ave = cur
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    after_score = boar_brawl(score, opponent_score)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    after_score = boar_brawl(score, opponent_score)
    after_score = sus_points(after_score + score)
    if (after_score - score >= threshold) :
        return 0
    else :
        return num_rolls
