...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        def separate_expressions(s):
            if s is nil:
                return nil
            else:
                return Pair(s.first, separate_expressions(s.rest))
        new_expressions = Pair(signature.rest, separate_expressions(expressions.rest))
        new_signature = signature.first
        new_lambda_procedure = do_lambda_form(new_expressions, env)
        env.define(new_signature, new_lambda_procedure)
...
def do_quote_form(expressions, env):
    ...
    def cal(s):
        if s is nil:
            ...
            return Pair(s.first, cal(s.rest))
    expressions = cal(expressions)
    return expressions
    # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    def init_names_Pair(s):
        if s is nil:
            return nil
        else:
            return Pair(s.first.first, init_names_Pair(s.rest))

    def init_vals_Pair(s):
        if s is nil:
            return nil
        else:
            return Pair(scheme_eval(s.first.rest.first, env), init_vals_Pair(s.rest))
    names = init_names_Pair(bindings)
    vals = init_vals_Pair(bindings)
