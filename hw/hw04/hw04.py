...
def balanced(m):
    ...
    def cal (m) :
        ...
        if is_mobile(m) :
            left_total_mass, right_total_mass = total_mass(end(left(m))), total_mass(end(right(m)))
            left_len, right_len = length(left(m)), length(right(m))
            left_torque, right_torque = left_total_mass * left_len, right_total_mass * right_len

            if left_torque != right_torque :
                is_balanced = False

            cal(end(left(m))), cal(end(right(m)))
...
def max_path_sum(t):
    ...
    def find_max (t) :
        if is_leaf(t) :
            ...
            return label(t) + max([find_max(item) for item in branches(t)])
